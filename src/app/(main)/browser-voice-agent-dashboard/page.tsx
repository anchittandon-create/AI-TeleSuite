...
import type { ActivityLogEntry, BrowserVoiceAgentActivityDetails, ScoreCallOutput, Product } from '@/types';
import { useProductContext } from '@/hooks/useProductContext';
import { Label } from '@/components/ui/label';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

interface HistoricalBrowserCallItem extends Omit<ActivityLogEntry, 'details'> {
  details: BrowserVoiceAgentActivityDetails;
}
...
export default function BrowserVoiceAgentDashboardPage() {
  ...
  const [selectedCall, setSelectedCall] = useState<HistoricalBrowserCallItem | null>(null);
  ...
  const salesCallHistory: HistoricalBrowserCallItem[] = useMemo(() => {
    if (!isClient) return [];
    return (activities || [])
      .filter(activity =>
      ...
      .map(activity => activity as HistoricalBrowserCallItem)
  ...
  const handleViewDetails = (item: HistoricalBrowserCallItem) => {
  ...
  const handleExportTable = (formatType: 'csv' | 'pdf' | 'doc') => {
    if (filteredHistory.length === 0) {
      toast({ title: "No Data", description: `No browser call history for '${productFilter}' to export.` });
      return;
    }
    try {
      ...
      const baseFilename = `browser_voice_call_history_${productFilter}_${timestamp}`;
      ...
      toast({ title: "Export Successful", description: `Browser call history exported as ${formatType.toUpperCase()}.` });
    } catch (error) {
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        <Alert variant="default" className="bg-blue-50 border-blue-200 text-blue-700">
            ...
            <AlertDescription className="text-xs">
              This dashboard displays logs of simulated sales calls initiated via the "Browser Voice Agent" module, which uses your browser's built-in voices. 
              Each entry includes the conversation transcript and the final call score.
            </AlertDescription>
        </Alert>
        ...
        {isClient ? (
          <Card className="shadow-md">
            <CardHeader>
                <CardTitle className="flex items-center"><BarChartHorizontalIcon className="mr-2 h-5 w-5 text-primary"/>Simulated Sales Call Logs (Browser)</CardTitle>
                <CardDescription>History of client-side voice call simulations. Click "View" for details.</CardDescription>
            </CardHeader>
            <CardContent>
                <ScrollArea className="h-[calc(100vh-460px)] md:h-[calc(100vh-400px)]">
                    <Table>
                        ...
                        {filteredHistory.length === 0 ? (
                            <TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">No browser call simulations logged for '{productFilter}' yet.</TableCell></TableRow>
                        ) : (
        ...
        {selectedCall && (
            ...
            <DialogContent className="sm:max-w-3xl md:max-w-4xl lg:max-w-5xl max-h-[90vh] flex flex-col p-0">
                ...
                <DialogTitle className="text-lg text-primary">Browser Call Simulation Details</DialogTitle>
                    ...
                    {selectedCall.details.input && (
                        <Card className="mb-4 bg-muted/30">
                            ...
                            <CardContent className="text-xs px-4 pb-3 space-y-1">
                                <p><strong>AI Agent Name (Simulated):</strong> {selectedCall.details.input.agentName || "Default AI"}</p>
                                <p><strong>Customer Name:</strong> {selectedCall.details.input.userName || "N/A"}</p>
                                ...
                                {selectedCall.details.input.voiceName && <p><strong>Simulated Voice:</strong> {selectedCall.details.input.voiceName}</p>}
                            </CardContent>
                        </Card>
                    )}
                            ...
                            <CardContent className="px-4 pb-3">
                                <Textarea value={selectedCall.details.fullTranscriptText} readOnly className="h-48 text-xs bg-background/50 whitespace-pre-wrap" />
                                 <div className="mt-2 flex gap-2">
                                     ...
                                     <Button variant="outline" size="xs" onClick={() => handleDownloadFile(selectedCall.details.fullTranscriptText!, `BrowserCall_${selectedCall.details.input.userName || 'User'}_transcript`, "transcript")}><Download className="mr-1 h-3"/>Download .txt</Button>
                                </div>
                            </CardContent>
                        </Card>
                    )}
