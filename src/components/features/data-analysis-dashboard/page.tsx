...
export default function DataAnalysisDashboardPage() {
  ...
  const filteredHistory = useMemo(() => {
      return dataAnalysisHistory.filter(item => item.product === selectedProduct);
  }, [dataAnalysisHistory, selectedProduct]);
  ...
  const handleExport = (formatType: 'csv' | 'pdf' | 'doc') => {
    if (filteredHistory.length === 0) {
      toast({
        variant: "default",
        title: "No Data",
        description: `There is no Data Analysis Report history for product '${selectedProduct}' to export.`,
      });
      return;
    }
    try {
      ...
      const baseFilename = `data_analysis_report_history_${selectedProduct}_${timestamp}`;
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title={`Data Analysis Report Dashboard - ${selectedProduct}`} />
