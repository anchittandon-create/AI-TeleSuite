...
export default function TrainingMaterialDashboardPage() {
  ...
  const filteredHistory = useMemo(() => {
      return trainingMaterialHistory.filter(item => item.product === selectedProduct);
  }, [trainingMaterialHistory, selectedProduct]);
  ...
  const handleExport = (formatType: 'csv' | 'pdf' | 'doc') => {
    if (filteredHistory.length === 0) {
      toast({
        variant: "default",
        title: "No Data",
        description: `There is no training material history for product '${selectedProduct}' to export.`,
      });
      return;
    }
    try {
      ...
      const baseFilename = `training_material_history_${selectedProduct}_${timestamp}`;
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title={`Training Material Dashboard - ${selectedProduct}`} />
      <main className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        ...
        {isClient ? (
          <TrainingMaterialDashboardTable history={filteredHistory} />
        ) : (
