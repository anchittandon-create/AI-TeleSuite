...
export async function analyzeData(input: DataAnalysisInput): Promise<DataAnalysisReportOutput> {
  return dataAnalysisReportFlow(input);
}
...
const dataAnalysisReportFlow = ai.defineFlow(
  ...
  async (input: DataAnalysisInput): Promise<DataAnalysisReportOutput> => {
    ...
    if (!output) {
      console.error("Data analysis report flow: Prompt returned null output for input:", input.userAnalysisPrompt);
      return {
        reportTitle: "Error Generating Analysis Report",
        executiveSummary: "The AI failed to generate an executive summary for the analysis.",
        keyMonthlyTrends: "AI failed to analyze monthly trends.",
        agentTeamPerformance: "AI failed to analyze agent/team performance.",
        cohortAnalysis: "AI failed to analyze cohort performance.",
        callHandlingEfficiency: "AI failed to analyze call handling efficiency.",
        leadQualityAndFollowUp: "AI failed to assess lead quality and follow-up.",
        incentiveEffectiveness: "AI failed to evaluate incentive effectiveness.",
        recommendationsWithDataBacking: [{ area: "Error", recommendation: "AI failed to generate recommendations.", dataBacking: "N/A" }],
        limitationsAndDisclaimer: "The AI analysis report generation process encountered an error. Please try again. This output is not a valid analysis report.",
    ...
    output.reportTitle = output.reportTitle || "Comprehensive Data Analysis Report";
    output.limitationsAndDisclaimer = output.limitationsAndDisclaimer || "This is an AI-generated analysis. The AI has NOT directly processed binary file content (Excel etc.). For CSV/TXT, only a sample may have been used for direct insights. User is responsible for verifying findings against their actual data and business context.";
    if (!output.executiveSummary) output.executiveSummary = "No executive summary generated.";
    if (!output.recommendationsWithDataBacking || output.recommendationsWithDataBacking.length === 0) {
        output.recommendationsWithDataBacking = [{ area: "General", recommendation: "No specific recommendations generated.", dataBacking: "N/A" }];
    }
    
    if (input.sampledFileContent && (!output.directInsightsFromSampleText || output.directInsightsFromSampleText.trim() === "")) {
        output.directInsightsFromSampleText = "The AI was instructed to analyze the provided text sample but did not return specific direct insights. It might have found the sample too brief or generic, but it was considered for the overall analysis.";
    }
