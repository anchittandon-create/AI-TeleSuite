  ...
  async (input: DataAnalysisInput): Promise<DataAnalysisReportOutput> => {
    try {
      ...
      if (!output) {
        ...
        // This specific error (null output from prompt) is less likely if the prompt itself doesn't error,
        // but good to keep for robustness.
        return {
          reportTitle: "Error: AI Prompt Returned No Output",
          executiveSummary: "The AI prompt completed but returned no structured output. This might indicate an issue with the prompt's ability to generate content for the given input, or a very unusual LLM response.",
          ...
          limitationsAndDisclaimer: "The AI analysis could not be completed as the prompt returned no data. Please review your input and try again. If the problem persists, it might be an internal AI configuration or model issue.",
      ...
      // This also helps ensure the returned object shape matches DataAnalysisReportSchema.
      const validatedOutput: DataAnalysisReportOutput = {
        ...
        directInsightsFromSampleText: output.directInsightsFromSampleText, // Keep as is, can be undefined
      ...
      // Ensure the error object matches the DataAnalysisReportOutput schema
      return {
