...
const DataAnalysisReportSchema = z.object({
  ...
  detailedAnalysis: z.object({
    dataReconstructionAndNormalizationSummary: z.string().optional().describe("A brief summary of how the AI *hypothetically* cleaned, reconstructed, or normalized the data tables based on the user's description of the data and its potential messiness. This section acknowledges the data preparation steps an analyst would take."),
    smartTableRecognitionSummary: z.string().optional().describe("Brief summary of how the AI *inferred* the purpose of different described data tables (e.g., CDR, Daily MIS, Source Dump) based on the column names and context provided by the user."),
...
This is your PRIMARY source of information about the data structure, contents (including any messiness like misaligned headers, merged rows, nulls like "NA" or "—"), specific file mappings, decoding rules for coded fields, and the user's analytical goals for this run.
"""
{{{userAnalysisPrompt}}}
"""
...
1.  **Data Reconstruction (Simulated Pre-analysis Cleanup)**:
    *   Based on the user's description of their data (including any mentioned malformed tables, misaligned headers, merged rows, repeated titles, varied column formats like date/currency/text/numeric, hidden headers, merged agent names/cohorts, nulls like "NA", "—"), explain briefly in the \`dataReconstructionAndNormalizationSummary\` section how you would hypothetically identify and correct these issues to prepare the data for analysis.

2.  **Table Normalization (Simulated)**:
    *   Describe in the \`dataReconstructionAndNormalizationSummary\` how you would reconstruct each described sheet or data source into clean, properly labeled tables. For example, you might discuss creating conceptual clean dataframes for agent-level data, call logs, lead data, and revenue summaries based on what the user has told you.

3.  **Smart Table Recognition (Based on User's Description)**:
    *   In the \`smartTableRecognitionSummary\` section, explain how you are inferring the purpose of different data tables/sheets described by the user. For instance, if the user describes columns like "Call Status", "Duration", "Follow-up", you'd state you're treating it as CDR data. If they mention "Revenue", "Agent", "Login Hours", you'd treat it as Daily MIS, and so on for "Cohort", "Lead ID", "Source" (Source Dump) or month names + revenue (Monthly Tracker).
...
4.  **KPI Calculation (Based on User's Description and Assumed Clean Data)**:
    *   From the *assumed* clean tables (derived from the user's description and your hypothetical cleaning), calculate or explain how you would calculate key KPIs. Populate the \`keyMetrics\` section with these. Use these definitions if applicable and if the user's data description supports them:
        *   Conversion Rate = (Interested + Subscribed outcomes) / Total leads or Total calls
        *   Avg Revenue per Call = Total Revenue / Connected Calls
        *   Lead Follow-up Rate = (# of CALLB or follow-up attempts) / Total Leads
        *   Connection Rate = (# Connected outcomes like 'INT', 'CALLB', 'ALREAD') / Total Calls
    *   If revenue is missing from the description, state you are inferring performance using proxy indicators like intent outcome distribution.
...
5.  **Insight Generation (From Assumed Clean Data)**:
    ...
    *   Highlight top trends, bottlenecks, and agent or cohort gaps.
    *   Suggest fixes (e.g., agent coaching, lead rerouting, incentive misalignment) in the \`recommendations\` section.
    *   Be proactive: if something seems off (e.g., very low call duration *as described by the user*), flag it as a red flag and suggest possible causes.
    ...
    try {
      if (!input.userAnalysisPrompt || input.userAnalysisPrompt.length < 50) {
        return {
            ...
            keyMetrics: [],
            detailedAnalysis: {
                dataReconstructionAndNormalizationSummary: "Input prompt insufficient.",
                smartTableRecognitionSummary: "Input prompt insufficient."
            },
