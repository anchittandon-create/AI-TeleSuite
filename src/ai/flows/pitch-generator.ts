...
const GeneratePitchInputSchema = z.object({
  product: z.enum(PRODUCTS).describe('The product to pitch (ET or TOI).'), // Updated
  customerCohort: z.string().describe('The customer cohort to target.'),
  etPrimePlanType: z.enum(ETPRIME_PLAN_TYPES).optional().describe('The specific ET plan duration (1-Year, 3-Year, 7-Year). Only applicable if product is ET.'), // Updated "ETPrime" to "ET"
});
...
4.  A main pitch body that flows well, is substantial, and expands on the value proposition, designed to last 2-3 minutes when spoken.
    - **Pricing for ET (The Economic Times Subscription)**:
        {{#if etPrimePlanType}}
        You have selected the {{etPrimePlanType}} plan for ET. Incorporate the following pricing into the pitch:
            ...
            - **3-Year Plan**: "For even greater savings, our 3-year ET plan is available at an effective price of only ₹153 per month. The total for three years is ₹5497. Plus, there's an extra ₹400 discount if you use a credit card. This is a fantastic 45% saving!"
            {{/if}}
            {{#if (eq etPrimePlanType "7-Year")}}
            - **7-Year Plan**: "To get the absolute best value, the 7-year ET plan works out to just ₹108 per month. The total billed amount for seven years is ₹9163. And you get an additional ₹500 off with credit card payments! This incredible offer also gives you a 45% discount."
            {{/if}}
        ...
        {{else}}
          {{#if (eq product "ET")}}
          - **Pricing for ET**: "For the most up-to-date and detailed ET pricing, including any special offers, please refer to our official document A4-27.pdf. I will now outline the general value and benefits, and your sales agent can provide the precise current subscription options based on this document."
          {{/if}}
        {{/if}}
    - **Pricing for TOI**:
        {{#if (eq product "TOI")}}
        ...
        {{/if}}
    - **General Pricing Note**: If specific pricing details are not available for the selected product/plan, create a placeholder statement instructing the sales agent to provide the current pricing information, mentioning the relevant document (A4-26.pdf for TOI, A4-27.pdf for ET) if applicable.
