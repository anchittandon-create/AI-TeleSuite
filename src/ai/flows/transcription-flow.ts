...
const transcriptionFlow = ai.defineFlow(
  ...
  async (input: TranscriptionInput) : Promise<TranscriptionOutput> => {
    ...
    const transcriptionPrompt = `You are an expert transcriptionist. Your task is to transcribe the provided audio of a conversation that can include a live agent, a customer, IVR menus, ringing, pre-call answers, or hold music.
...
4.  **System & Non-Human Audio Events:** Clearly differentiate IVR menus, ringing, hold music, silence, or any other non-human audio. Within the appropriate time block, add a separate line containing one of these tags (no colon afterwards): [PRECALL_ANSWER], [RINGING], [IVR_PROMPT], [IVR_MENU], [HOLD_TONE], [MUSIC], [SILENCE]. You may append a short description after a hyphen if helpful. These events must NEVER be attributed to AGENT or USER.
