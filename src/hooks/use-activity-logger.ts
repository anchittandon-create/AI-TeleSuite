...
export const MAX_ACTIVITIES_TO_STORE = 50; // Limit the number of activities - EXPORTED
...
export function useActivityLogger() {
  ...
  const logActivity = useCallback((activity: Omit<ActivityLogEntry, 'id' | 'timestamp' | 'agentName'>) => {
    // console.log("useActivityLogger: logActivity called with:", activity, "Current profile:", currentProfile);
    ...
    setActivities(prevActivities => {
      ...
      // console.log("useActivityLogger: Current activities after update:", finalActivities.length, "items. First item ID:", finalActivities[0]?.id);
